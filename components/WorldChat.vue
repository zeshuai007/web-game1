  ...
  <div class="h-full flex flex-col">
    <div class="flex items-center justify-between gap-3 mb-3">
      <div>
        ...
        <p class="text-xs text-ink-500 mt-1">当前在线 {{ chat.onlineCount }} 人</p>
      </div>
      <span v-if="chat.isRealtimeEnabled && chat.connectionState !== 'connected'" class="text-xs px-2 py-1 rounded-full border border-gold-700/30 bg-gold-950/40 text-gold-300">
        连接中
      </span>
    </div>
    ...
    <div class="flex-1 min-h-0 rounded-lg bg-ink-950/60 border border-ink-800 overflow-hidden flex flex-col">
      <div ref="scroller" class="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        <div v-if="chat.worldMessages.length === 0" class="text-center text-sm text-ink-500 py-10">
          江湖寂静，尚无人发言。
        </div>

        <div v-for="(message, index) in chat.worldMessages" :key="message.id || `${message.timestamp}-${index}`"
      ...
      <div v-if="chat.isRealtimeEnabled && chat.connectionState === 'disconnected'" class="px-4 py-2 text-xs text-gold-200 bg-gold-950/50 border-t border-gold-700/20">
        连接断开，正在重连...
      </div>
      ...
      <form class="border-t border-ink-800 p-3 flex gap-2" @submit.prevent="handleSend">
        <input
          ...
          type="text"
          maxlength="200"
          :disabled="sending || !canSpeak || (chat.isRealtimeEnabled && chat.connectionState !== 'connected')"
...
const placeholder = computed(() => {
  ...
  if (chat.isRealtimeEnabled.value && chat.connectionState.value !== 'connected') return '聊天连接中...'
  return '输入江湖传音...'
})
