...
大境界突破概率（`config_realms.breakthrough_chance`，前期体验优化后）：凝气 **60%**、筑基 **50%**、结丹 30%、元婴 25%、化神 20%、婴变 15%（问鼎为终点），破境丹 +20%。

前期（凝气/筑基）附加规则：失败保留 50% 进度；连败保底每次 +5%、上限 +20%，成功后清零。数值由 `config_realms` 表管理，详见「修炼前期体验优化」章节。

**产出速率与加成体系**（试玩校准后，`config_realms`）：凝气/筑基灵气与灵石速率 **90/分钟**（首次到筑基瓶颈 ≈15 分钟，达成前期目标）；结丹及以后维持原长线节奏。

结算公式：`基础速率 ＋ 装备加成（穿戴合计）→ × 修炼丹 buff → × 宗门等级加成`。装备品质加成：凡器 +2% / 法器 +5% / 宝器 +10% / 灵器 +20% / 仙器 +40%；宗门每级 +2%。奇遇触发有 10 分钟冷却。
